...
|---|---|
| A contract property written for a window is remembered into a window by whoever adds it. `InfoAge`, `InstallLocation` and `Notice` were not. | A unit test over `WorkspaceSummary`, `LiveAppSessionSummary`, `WorkerActivity` and `ProjectStatus`: every public property must be named somewhere under `src/RoseMcp.Ui.Core`, `src/RoseMcp.Tray` or `src/RoseMcp.Inspector`, or be listed in an explicit `NotShownDeliberately` set with a reason string. A one-line source scan; the opt-out list *is* the documentation of what is agent-only. |
