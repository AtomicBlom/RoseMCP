...
|---|---|
| ~~A contract property written for a window is remembered into a window by whoever adds it.~~ **Done, PR #TIER0**, as `ProducedFactTests`. Seeded with the 19 properties no window names -- 13 of them `ProjectStatus`, which no window renders at all, and 18 of which are card 22's -- so the exemption list is that card's worklist and an empty one is its definition of done. | The scan is a word match over the three windows' sources, which cannot tell rendering from a stray mention. Deliberate: it catches the failure it is for, a property nobody named anywhere. |
...
1. ~~Who is the pixel lens for?~~ **Answered during review, and it reversed a recommendation.** The
   Windows magnifier applies bilinear filtering that cannot be disabled, so it can neither give an
   accurate colour nor reveal a one-pixel gap between a `Border`'s corner radius and the background
   at 8x. Nearest neighbour is the only way to get either. The remaining question is narrower: **is
   anything else in the product justified by a platform limitation that is not written down?** The
   magnifier came within one review of being deleted because that reason lived only in the author's
   head, and the same review had no way to detect the pattern elsewhere.
