...
|---|---|
| ~~A contract property written for a window is remembered into a window by whoever adds it.~~ **Done, PR #295**, as `ProducedFactTests`, whose exemption list is card 22's worklist. | The scan is a word match over the three windows' sources: it cannot tell rendering from a stray mention, and still catches a property nobody named anywhere. |
