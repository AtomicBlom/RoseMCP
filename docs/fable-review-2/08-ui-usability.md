...
|---|---|
| ~~A contract property written for a window is remembered into a window by whoever adds it.~~ **#295.** A test fails when one is not, and its exemption list is card 22's worklist. | A word match over the windows: it cannot tell rendering from a stray mention, and still catches a property nobody named anywhere. |
