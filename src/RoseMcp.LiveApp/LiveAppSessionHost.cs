...
public sealed class LiveAppSessionHost(LiveAppOptions options, ILogger<LiveAppSessionHost> logger) : IHostedService
{
	...
	/// <summary>The XAML face of this session, and the diagnostics session that answers it.</summary>
	private readonly TargetXaml _xaml = new(logger);
	...
	/// <summary>
	/// The facts a XAML request needs about the target, taken together so one request sees one
	/// consistent answer. The debug session is picked up first, outside the gate, because asking it
	/// for its stop is a read of its own state under its own lock.
	/// </summary>
	private XamlAsk Asking()
	{
		var session = Attached();

		lock (_gate)
		{
			return new XamlAsk(_targetProcessId, _uwpInstallLocation, session, _events);
		}
	}

	/// <inheritdoc cref="TargetXaml.ReadTree"/>
	public LiveXamlTree ReadXamlTree(string? rootName, int offset, int limit)
		=> _xaml.ReadTree(Asking(), rootName, offset, limit);

	/// <inheritdoc cref="TargetXaml.ReadProperties"/>
	public LiveXamlProperties ReadXamlProperties(ulong handle, bool includeDefaults)
		=> _xaml.ReadProperties(Asking(), handle, includeDefaults);

	/// <inheritdoc cref="TargetXaml.EnterSelectMode"/>
	public LiveXamlSelection EnterXamlSelectMode(
		bool includeAllElements,
		bool justMyXaml,
		bool arm = true,
		string mode = "select")
		=> _xaml.EnterSelectMode(Asking(), includeAllElements, justMyXaml, arm, mode);

	/// <inheritdoc cref="TargetXaml.ReadSelection"/>
	public LiveXamlSelection ReadXamlSelection() => _xaml.ReadSelection();

	/// <inheritdoc cref="TargetXaml.ClearSelection"/>
	public LiveXamlSelection ClearXamlSelection() => _xaml.ClearSelection(Asking());

	/// <inheritdoc cref="TargetXaml.SelectElement"/>
	public LiveXamlSelection SelectXamlElement(ulong handle) => _xaml.SelectElement(Asking(), handle);

	/// <inheritdoc cref="TargetXaml.Apply"/>
	public LiveXamlApplyResult ApplyXaml(string? oldXaml, string? newXaml, string? filePath)
		=> _xaml.Apply(Asking(), oldXaml, newXaml, filePath);
