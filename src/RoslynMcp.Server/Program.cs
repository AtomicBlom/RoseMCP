...
internal static class Program
{
	private static async Task<int> Main(string[] args)
	{
		...
		catch (ArgumentException exception)
		{
			...
			await Console.Error.WriteLineAsync(
				"usage: RoslynMcp.Server [--transport stdio|http] [--host <address>] [--port <n>] "
	...
	private static async Task<int> RunStdioAsync(ServerOptions options)
	{
		...
		builder.Services
			.AddRoslynMcpBroker(broker => Apply(options, broker))
	...
	private static async Task<int> RunHttpAsync(ServerOptions options)
	{
		...
		builder.Services.AddRoslynMcpBroker(broker => Apply(options, broker)).WithHttpTransport();

		var application = builder.Build();

		var token = Environment.GetEnvironmentVariable("ROSLYNMCP_TOKEN");
